...
def read_protocols_mobile_saude() -> str:
    ...
    def process_protocol() -> list:
        ...
        refund_id = match(r'(?<=value=")(.*?)(?=")', protocol[0]).strip()
        protocol_id = protocol[1]
        status_id = match(r'(?<=data-status=")(.*?)(?=")', protocol[0]).strip()
        refund_type = protocol[2]
        refund_qty = protocol[3]
        protocol_date = protocol[4]
        refund_value = protocol[5]
        status_desc = re.sub(r"<[^>]*>", "", protocol[6])    
        PEG = ""
        holder_name = ""
        phone_number = ""
        holder_cpf = ""
    ...
    if len(df_reprocess_protocols) <= int(global_parameters["mobilesaude.total_records"]):
        ...
        protocols_with_error = set(df_protocols_with_error['protocol_id'].values)

        # 
        total_records = int(global_parameters["mobilesaude.total_records"]) - len(df_reprocess_protocols)
        
        protocols = get_protocols(session)

        # Filtra o array removendo os que já existem no protocols_with_error
        protocols = [linha for linha in protocols if linha[1] not in protocols_with_error]
        ...
        for protocol in protocols:
            ...
            logging.info(f'Lendo protocolo {protocol[1]} {protocols_count}/{len(protocols)}')

            # verifica se o protocolo já está na lista de reprocessamento
            if not protocol[1] in df_reprocess_protocols['protocolo'].values:
                array.append(process_protocol())
            else:
                logging.info(f'Protocolo {protocol[1]} já estava na lista de reprocessamento')
